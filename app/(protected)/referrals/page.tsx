...
function ReferralTimeline({
  ...
  if (error) {
    return (
      <div
        className="flex items-start gap-2 rounded-md border border-warning/30 bg-warning/10 px-3 py-3 text-xs text-warning-foreground"
        role="alert"
      >
        <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
        Unable to load timeline events: {error}
      </div>
    );
  }
  ...
  if (events.length === 0) {
    return (
      <p className="rounded-md bg-muted/40 px-3 py-3 text-xs text-muted-foreground">
        No timeline events are available for this referral.
      </p>
    );
  }

  return (
    <ol className="relative ml-2 border-l border-border/80 pl-5">
      {events.map((event) => (
        <li key={event.id} className="relative pb-4 last:pb-0">
          <span className="absolute -left-[1.34rem] top-1 h-2.5 w-2.5 rounded-full border-2 border-background bg-primary" />
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            ...
            {event.recorded_by
              ? ` · ${
                  recordedByNames.get(event.recorded_by) ??
...
function ReferralForm({
  ...
  saving: boolean;
  error: string | null;
  onChange: (field: keyof ReferralForm, value: string) => void;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
  onCancel: () => void;
}) {
  const patient = patients.find((item) => item.id === form.patientId);

  const sourceFacility = facilities.find(
    (facility) => facility.id === patient?.facility_id
  );
  ...
  return (
    <Card className="border-primary/20 shadow-sm">
      <CardHeader className="flex-row items-start justify-between space-y-0 border-b border-border/70">
        <div>
          <CardTitle className="text-lg">New referral</CardTitle>
          <CardDescription className="mt-1">
            Create a referral from the patient&apos;s assigned facility.
          </CardDescription>
        </div>
        ...
        <CardContent className="space-y-5 pt-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                onChange('patientId', event.target.value)
              }
          ...
          <div className="rounded-md border border-border/70 bg-muted/30 p-3 text-sm">
            ...
            <span className="text-muted-foreground">
              {sourceFacility?.name ??
                (patient
                  ? 'Patient has no assigned facility'
                  : 'Selected patient facility will appear here')}
            </span>
          </div>

          <div className="space-y-2">
            <Label htmlFor="referral-destination">
            ...
            <select
              ...
              onChange={(event) =>
                onChange('destinationFacilityId', event.target.value)
              }
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            >
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Record why the referral is being made"
            />
          </div>
...
function ReferralCard({
  ...
  const [recordedByNames, setRecordedByNames] = useState(
  ...
  const loadEvents = async () => {
    ...
    if (eventResult.error) {
      ...
      const recorderIds = Array.from(
        new Set(
          loadedEvents
            .map((event) => event.recorded_by)
            .filter((id): id is string => Boolean(id))
        )
      );

      if (recorderIds.length > 0) {
        const recorderResult = await supabaseClient
          .from('profiles')
          .select('id, full_name')
          .in('id', recorderIds);
      ...
      <CardContent className="p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
              ...
              Created {formatDate(referral.referral_date)}
            </p>
          </div>
          ...
          >
            {labelFor(statusOptions, referral.status)}
          </span>
        </div>
        ...
        <div className="mt-5 grid gap-4 border-t border-border/70 pt-4 text-sm sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 text-foreground">
              {labelFor(priorityOptions, referral.priority)}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 border-t border-border/70 pt-4 text-xs text-muted-foreground sm:grid-cols-2">
          <span>
            Referral date: {formatDate(referral.referral_date)}
          </span>
        ...
        <div className="mt-4 border-t border-border/70 pt-4">
          <button
            ...
            className="flex w-full items-center justify-between text-left text-sm font-semibold text-foreground hover:text-primary"
          ...
          {timelineOpen && (
            <div className="mt-4">
              <ReferralTimeline
                ...
                error={eventsError}
                recordedByNames={recordedByNames}
              />
            </div>
          )}
        </div>
        ...
        {canManage && (
          <div className="mt-4 flex flex-wrap gap-2 border-t border-border/70 pt-4">
            ...
            {statusOptions
              .filter((option) => option.value !== referral.status)
              .map((option) => (
                <Button
                  key={option.value}
                  type="button"
                  ...
                  disabled={updating}
                  onClick={() =>
                    onStatusChange(referral, option.value)
                  }
                >
                  {updating && (
...
export default function ReferralsPage() {
  ...
  const loadData = async () => {
    ...
    if (isPatient) {
      ...
      patientRecord = patientResult.data as Patient;
    }

    const referralQuery = supabaseClient
      .from('referrals')
      .select('*')
      .order('referral_date', { ascending: false });

    const referralResult = patientRecord
      ? await referralQuery.eq('patient_id', patientRecord.id)
      : await referralQuery;

    if (referralResult.error) {
      setError(referralResult.error.message);
      setLoading(false);
      return;
    }

    if (isPatient) {
      setPatients(patientRecord ? [patientRecord] : []);
    } else {
      const patientResult = await supabaseClient
        .from('patients')
        .select('*')
    ...
    const facilityResult = await supabaseClient
      .from('facilities')
      .select('*')
  ...
  const filteredReferrals = useMemo(
    () =>
      referrals.filter((referral) => {
        ...
        const patientName =
          patientNames.get(referral.patient_id) ?? '';
        ...
        return (
          ...
          (destinationFilter === 'all' ||
            referral.destination_facility_id ===
              destinationFilter) &&
          (!dateFilter ||
            referral.referral_date.slice(0, 10) ===
              dateFilter)
        );
      }),
    [
  ...
  const saveReferral = async (
    ...
    if (
      !patient ||
      !form.destinationFacilityId ||
      !form.reason.trim()
    ) {
      setFormError(
        'Select a patient and destination facility, and enter a referral reason.'
      );
      setSaving(false);
      return;
    }
    ...
    if (!patient.facility_id) {
      setFormError(
        'This patient has no assigned source facility. Assign a facility on the patient record before creating a referral.'
      );
      setSaving(false);
      return;
    }
    ...
    if (
      ...
      setFormError(
        'Destination facility must differ from the source facility.'
      );
      setSaving(false);
      return;
    }
    ...
    if (saveError) {
      ...
      setNotice('Referral created successfully.');
      setForm(emptyForm);
      setFormOpen(false);
      await loadData();
    }
  ...
  const changeStatus = async (
    ...
    if (updateError) {
      setError(
        `Unable to update referral status: ${updateError.message}`
      );
    } else {
      setNotice(
        `Referral marked ${labelFor(
          statusOptions,
          status
        ).toLowerCase()}.`
      );
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground">
            {isPatient ? t('referrals') : 'Closed-Loop Referrals'}
          </h1>

          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            {isPatient
              ? t('referrals')
              : 'Track referrals from creation through receiving-facility acceptance and completion.'}
          </p>
        </div>
      ...
      {!canManage && (
        <div className="mb-6 flex items-start gap-2 rounded-md border border-primary/20 bg-primary/5 p-3 text-sm text-muted-foreground">
          ...
          {isPatient
            ? 'Only your own referral records are shown.'
            : 'You have read-only referral access. Existing Supabase RLS policies remain the final authority.'}
        </div>
      )}
        ...
        <section className="min-w-0 space-y-5">
            ...
            <CardContent className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  aria-label="Filter referrals by date"
                />
                ...
                <Button
                  ...
                  aria-label="Clear referral filters"
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
            ...
            >
              ...
              <div>
                <p className="font-semibold">
                  {isPatient ? t('error') : 'Unable to load referrals'}
                </p>
                ...
                <Button
                  ...
                  className="mt-3"
                >
                  {isPatient ? t('tryAgain') : 'Try again'}
                </Button>
              </div>
            </div>
          )}
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              {isPatient ? t('referrals') : 'Referral list'}
            </h2>
          ...
          {loading && !error && (
            <div className="flex min-h-48 items-center justify-center rounded-lg border border-border/70 bg-card">
              ...
              <span className="ml-2 text-sm text-muted-foreground">
                {isPatient ? t('loading') : 'Loading referrals...'}
              </span>
            </div>
          )}
            ...
            filteredReferrals.length === 0 && (
              <div className="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card px-6 text-center">
                ...
                <h3 className="mt-3 font-semibold text-foreground">
                  {isPatient ? t('empty') : 'No referrals found'}
                </h3>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  ...
                  destinationFilter !== 'all'
                    ...
                    : canManage
                    ? 'Create a referral to begin tracking a patient journey.'
                    : 'Your referrals will appear here when a healthcare worker creates one for you.'}
                </p>
              </div>
            )}
            ...
            <Card className="hidden border-dashed border-border/80 bg-card/50 lg:block">
              <CardContent className="flex flex-col items-center px-6 py-10 text-center">
                ...
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  Every referral remains visible until its receiving and completion status is recorded.
                </p>
