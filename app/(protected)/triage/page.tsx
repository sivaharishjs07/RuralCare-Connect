        ...
        <CardContent className="space-y-5 pt-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <select id="triage-risk" required value={form.severity} onChange={(event) => onChange('severity', event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
...
export default function TriagePage() {
  ...
  const saveAssessment = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setFormError(null);
    setNotice(null);
    if (!form.patientId || !form.symptoms.trim() || !form.severity || !form.riskScore || !form.recommendation.trim()) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-2 text-sm font-semibold uppercase tracking-wider text-primary">Clinical workflow</p>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Digital Triage</h1>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">Record and review professional-confirmed triage assessments for existing patients.</p>
        </div>
      ...
      {!canCreate && <div className="mb-6 flex items-start gap-2 rounded-md border border-primary/20 bg-primary/5 p-3 text-sm text-muted-foreground"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0 text-primary" />You have read-only access to triage assessments. Healthcare staff and doctors can record new assessments.</div>}

      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,25rem)]">
        <section className="min-w-0 space-y-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="text-lg font-semibold text-foreground">Assessment history</h2><p className="text-sm text-muted-foreground">{loading ? 'Loading assessments...' : `${filteredAssessments.length} assessment${filteredAssessments.length === 1 ? '' : 's'} shown`}</p></div>
